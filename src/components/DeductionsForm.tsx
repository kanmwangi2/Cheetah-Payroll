...
const DeductionsForm: React.FC<{ companyId: string; onAdded: () => void }> = ({ companyId, onAdded }) => {
  ...
  const validate = () => {
    ...
    if (!form.amount || isNaN(Number(form.amount)) || Number(form.amount) < 0) errs.amount = 'Amount must be a non-negative number';
    if (!form.staff_id.trim()) errs.staff_id = 'Employee ID is required';
    if (form.balance && (isNaN(Number(form.balance)) || Number(form.balance) < 0)) errs.balance = 'Balance must be a non-negative number';
    return errs;
  };
  ...
  return (
    <form className="deductions-form" onSubmit={handleSubmit} autoComplete="off">
      ...
      <div className="form-row">
        ...
        {fieldErrors.type && <div className="field-error" id="type-error">{fieldErrors.type}</div>}
      </div>
      <div className="form-row">
        <label>
          Amount
      ...
      <div className="form-row">
        ...
        {fieldErrors.staff_id && <div className="field-error" id="staffid-error">{fieldErrors.staff_id}</div>}
      </div>
      <div className="form-row">
        <label>
          Balance
        ...
        {fieldErrors.balance && <div className="field-error" id="balance-error">{fieldErrors.balance}</div>}
      </div>
      <div className="form-row">
