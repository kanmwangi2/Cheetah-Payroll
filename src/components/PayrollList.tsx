...
const PayrollList: React.FC<{ companyId: string }> = ({ companyId }) => {
  ...
  const validate = () => {
    ...
    if (!form.gross || isNaN(Number(form.gross)) || Number(form.gross) <= 0) errs.gross = 'Gross must be positive number';
    if (!form.basic || isNaN(Number(form.basic)) || Number(form.basic) < 0) errs.basic = 'Basic must be non-negative number';
    if (form.transport && (isNaN(Number(form.transport)) || Number(form.transport) < 0)) errs.transport = 'Transport must be non-negative number';
    if (form.otherDeductions && (isNaN(Number(form.otherDeductions)) || Number(form.otherDeductions) < 0)) errs.otherDeductions = 'Other deductions must be non-negative number';
    return errs;
  };
  ...
  const filtered = payrolls.filter(p => {
    return !search ||
      String(p.gross).includes(search) ||
      String(p.finalNet).includes(search);
  });
  ...
  return (
    <div className="payroll-list">
      ...
      <div className="payroll-form-wrapper">
        ...
        <div className="payroll-form">
          <div className="form-row">
            ...
            <input
              ...
              inputMode="decimal"
            />
            {fieldErrors.gross && <div className="field-error" id="gross-error">{fieldErrors.gross}</div>}
          </div>
          <div className="form-row">
            ...
            <input
              ...
              inputMode="decimal"
            />
            {fieldErrors.basic && <div className="field-error" id="basic-error">{fieldErrors.basic}</div>}
          </div>
          <div className="form-row">
            ...
            <input
              ...
              inputMode="decimal"
            />
            {fieldErrors.transport && <div className="field-error" id="transport-error">{fieldErrors.transport}</div>}
          </div>
          <div className="form-row">
            <label>
              Other Deductions
            ...
            {fieldErrors.otherDeductions && <div className="field-error" id="otherDeductions-error">{fieldErrors.otherDeductions}</div>}
          </div>
          <div className="form-row">
        ...
        {result && (
          <div className="payroll-calc-result">
            ...
            <button className="primary-btn" onClick={handleCreate}>Save Payroll</button>
          </div>
        )}
      </div>
      <PayrollImportExport companyId={companyId} onImported={() => setRefresh(r => r + 1)} payrolls={payrolls} />
      <h3>Payroll List</h3>
      ...
      {filtered.length === 0 ? (
        <div className="payroll-empty" aria-live="polite">No payrolls found.</div>
      ) : (
