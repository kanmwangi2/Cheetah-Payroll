...
import { fetchPayroll } from '../fetchPayroll';
import { submitPayroll, approvePayroll, rejectPayroll, PayrollStatus } from '../payroll';

// TODO: Replace with real user/role context
const mockUser = { id: 'demo-user', role: 'company_admin' };

export default function ApprovalWorkflow({ payrollId, companyId }: { payrollId: string, companyId: string }) {
  ...
  const canSubmit = payroll && payroll.status === 'draft' && ['payroll_preparer', 'company_admin', 'app_admin', 'primary_admin'].includes(mockUser.role);
  const canApprove = payroll && payroll.status === 'submitted' && ['company_admin', 'app_admin', 'primary_admin'].includes(mockUser.role);
  ...
  return (
    <div className="approval-workflow">
      ...
      {payroll && (
        <div>
          <div>Status: <b>{payroll.status || 'draft'}</b></div>
          {payroll.status === 'rejected' && (
            <div style={{ color: 'crimson' }}>Rejected: {payroll.rejectionReason || 'No reason provided'}</div>
          )}
            ...
            {canReject && (
              <span>
                <input
                  ...
                  style={{ marginRight: 4 }}
                />
                <button onClick={handleReject} disabled={!rejectReason}>Reject</button>
              </span>
            )}
          </div>
